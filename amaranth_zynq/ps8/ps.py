...
class PsZynqMP(wiring.Component):
    ...
    EMIOWDT0: Out(WDT())
    EMIOWDT1: Out(WDT())
    EMIOENET0: Out(ENET(gmii=True, mdio=True))
    EMIOENET1: Out(ENET(gmii=True, mdio=True))
    EMIOENET2: Out(ENET(gmii=True, mdio=True))
    EMIOENET3: Out(ENET(gmii=True, mdio=True))
    ...
    def __init__(self):
        ...
        for name, sz in self._BIDIRECTIONAL_PORTS:
            # setattr(self, name, IOPort(sz, name=name))
    ...
    def _get_instance_ports(self):
        ...
        for name, sz in self._BIDIRECTIONAL_PORTS:
            ports['o_' + name] = getattr(self, name)
